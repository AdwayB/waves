...
const getBulkUploadTemplate = async (): Promise<APIResponse> => {
  let response: AxiosResponse;
  try {
    response = await eventsAPI.get('/bulk-upload-template', {
      responseType: 'blob',
    });
  } catch (error: unknown) {
    const requestError: AxiosError = error as AxiosError;
...
const uploadBulkEventData = async (connectionId: string, file: FormData): Promise<APIResponse> => {
  let response: AxiosResponse;
  try {
    response = await eventsAPI.post(`/bulk-upload?connectionId=${connectionId}`, file);
  } catch (error: unknown) {
    const requestError: AxiosError = error as AxiosError;
    return { status: requestError.response?.status ?? 500, data: requestError.response?.data };
  }
